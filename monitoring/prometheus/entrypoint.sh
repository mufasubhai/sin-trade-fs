...
export SIN_TRADE_GRAFANA_HOST="${SIN_TRADE_GRAFANA_HOST:-grafana:3000}"
export SIN_TRADE_FE_HOST="${SIN_TRADE_FE_HOST:-frontend:5173}"  

# Substitute environment variables in the prometheus config template
envsubst '${SIN_TRADE_BE_HOST} ${SIN_TRADE_DS_HOST} ${SIN_TRADE_GRAFANA_HOST} ${SIN_TRADE_FE_HOST} ${SCRAPE_SCHEME}' < /etc/prometheus/prometheus.yml.template > /tmp/prometheus.yml
